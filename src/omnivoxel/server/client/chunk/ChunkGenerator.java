...
public class ChunkGenerator {
    ...
    public byte[] generateChunk(ChunkPosition chunkPosition) {
        List<ServerBlock> palette = new ArrayList<>();
        int[] chunk = new int[ConstantGameSettings.BLOCKS_IN_CHUNK * 2];
        ...
        for (int x = -1; x < ConstantGameSettings.CHUNK_WIDTH + 1; x++) {
            for (int z = -1; z < ConstantGameSettings.CHUNK_LENGTH + 1; z++) {
                for (int y = -1; y < ConstantGameSettings.CHUNK_HEIGHT + 1; y++) {
                    ServerBlock block = worldDataService.getBlockAt(chunkPosition, x, y, z);
        ...
        int paletteLength = 0;
        for (ServerBlock serverBlock : palette) {
            byte[] blockBytes = serverBlock.getBytes();
    ...
    private record BlockIDCount(int blockID, int count) {
        @Override
