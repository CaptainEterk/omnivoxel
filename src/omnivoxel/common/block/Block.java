package omnivoxel.common.block;
...
public record Block(String id, int[] blockState) implements ServerItem {
    ...
    public byte @NotNull [] getBytes() {
        ...
        if (blockState != null) {
            ...
            byte[] bytes = new byte[2 + idBytes.length + 2];
            bytes[0] = (byte) ((idBytes.length >> 8) & 0xFF);
            bytes[1] = (byte) (idBytes.length & 0xFF);
            System.arraycopy(idBytes, 0, bytes, 2, idBytes.length);
            return bytes;
    ...
    public omnivoxel.world.block.Block getBlock() {
        return new omnivoxel.world.block.Block(id, blockState);
    }
}