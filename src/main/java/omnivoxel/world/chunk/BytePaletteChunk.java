...
public class BytePaletteChunk<B> implements Chunk<B> {
    private final byte[] blocks;
    private final RotationChunk rotations;
    private final List<B> palette;
    private final Map<B, Byte> paletteIndex;
    private final int lod;

    public BytePaletteChunk(int lod) {
        this.lod = lod;
        this.blocks = new byte[ConstantCommonSettings.BLOCKS_IN_CHUNK >> (lod * 3)];
        this.rotations = new SingleRotationChunk((byte) 0, lod);
        this.palette = new ArrayList<>();
        this.paletteIndex = new HashMap<>();
    }

    public BytePaletteChunk(Chunk<B> chunk, int lod) {
        this.lod = lod;
        this.palette = new ArrayList<>();
        this.paletteIndex = new HashMap<>();
    ...
    private byte[] extractBlocks(Chunk<B> chunk) {
        byte[] blocks = new byte[ConstantCommonSettings.BLOCKS_IN_CHUNK >> (lod * 3)];
        ...
        for (int x = 0; x < ConstantCommonSettings.CHUNK_WIDTH >> lod; x++) {
            for (int z = 0; z < ConstantCommonSettings.CHUNK_LENGTH >> lod; z++) {
                for (int y = 0; y < ConstantCommonSettings.CHUNK_HEIGHT >> lod; y++) {
                    ...
                    Byte index = paletteIndex.get(block);
                    if (index == null) {
                        index = (byte) palette.size();
                        palette.add(block);
    ...
    public Chunk<B> setBlock(int x, int y, int z, B block) {
        ...
        if (palette.size() > Byte.MAX_VALUE - 2) {
            return new ShortPaletteChunk<>(this, lod);
        }
