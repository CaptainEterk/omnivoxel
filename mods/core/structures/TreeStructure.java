...
public class TreeStructure extends Structure {
    ...
    public @NotNull Structure initBlocks(BlockService blockService) {
        for (int y = 0; y < 5; y++) {
            blocks.put(new Position3D(2, y, 2), new PriorityServerBlock(blockService.getBlock("core:log_block", null), PriorityServerBlock.Priority.STRUCTURE_PRIMARY));
        }
        for (int x = 0; x < 5; x++) {
            for (int z = 0; z < 5; z++) {
                if (!((x == 0 || x == 4) && (z == 0 || z == 4)) && !(x == 2 && z == 2)) {
                    for (int y = 0; y < 1; y++) {
                        blocks.put(new Position3D(x, y + 3, z), new PriorityServerBlock(blockService.getBlock("core:leaf_block", null), PriorityServerBlock.Priority.DECORATION));
                    }
                }
            }
        }
